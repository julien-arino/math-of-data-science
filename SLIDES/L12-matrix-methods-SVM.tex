...
\maxFrameImage{FIGS/L12-plot-svm-example-1.pdf}

% --- Base R SVM before using e1071 ---
...
\begin{frame}[fragile]{R (base): fit SVM via hinge-loss GD}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Use X, y created above (x: matrix, y: \{-1,1\})}
\hldef{X} \hlkwb{<-} \hldef{x}
\hldef{Y} \hlkwb{<-} \hlkwd{as.numeric}\hldef{(y)}
\hldef{n} \hlkwb{<-} \hlkwd{nrow}\hldef{(X); p} \hlkwb{<-} \hlkwd{ncol}\hldef{(X)}

\hlcom{# Initialize parameters}
\hldef{w} \hlkwb{<-} \hlkwd{rep}\hldef{(}\hlnum{0}\hldef{, p)}
\hldef{b} \hlkwb{<-} \hlnum{0}
\hldef{lambda} \hlkwb{<-} \hlnum{0.05}   \hlcom{# L2 regularization strength (~ 1/(2nC))}
\hldef{lr} \hlkwb{<-} \hlnum{0.1}        \hlcom{# learning rate}
\hldef{epochs} \hlkwb{<-} \hlnum{5000}

\hlkwa{for} \hldef{(t} \hlkwa{in} \hlnum{1}\hlopt{:}\hldef{epochs) \{}
    \hldef{margins} \hlkwb{<-} \hldef{Y} \hlopt{*} \hldef{(}\hlkwd{as.vector}\hldef{(X} \hlopt{%*%} \hldef{w)} \hlopt{-} \hldef{b)}
    \hldef{active} \hlkwb{<-} \hldef{margins} \hlopt{<} \hlnum{1}  \hlcom{# points violating margin}
    \hlkwa{if} \hldef{(}\hlkwd{any}\hldef{(active)) \{}
        \hldef{grad_w} \hlkwb{<-} \hlnum{2} \hlopt{*} \hldef{lambda} \hlopt{*} \hldef{w} \hlopt{-} \hlkwd{colSums}\hldef{( (Y[active]} \hlopt{*} \hldef{X[active, ,} \hlkwc{drop} \hldef{=} \hlnum{FALSE}\hldef{]) )} \hlopt{/} \hldef{n}
        \hldef{grad_b} \hlkwb{<-}  \hlkwd{sum}\hldef{(Y[active])} \hlopt{/} \hldef{n}
    \hldef{\}} \hlkwa{else} \hldef{\{}
        \hldef{grad_w} \hlkwb{<-} \hlnum{2} \hlopt{*} \hldef{lambda} \hlopt{*} \hldef{w}
        \hldef{grad_b} \hlkwb{<-} \hlnum{0}
    \hldef{\}}
    \hlcom{# parameter update}
    \hldef{w} \hlkwb{<-} \hldef{w} \hlopt{-} \hldef{lr} \hlopt{*} \hldef{grad_w}
    \hldef{b} \hlkwb{<-} \hldef{b} \hlopt{-} \hldef{lr} \hlopt{*} \hldef{grad_b}
    \hlcom{# mild decay for stability}
    \hlkwa{if} \hldef{(t} \hlopt{%%} \hlnum{1000} \hlopt{==} \hlnum{0}\hldef{) lr} \hlkwb{<-} \hldef{lr} \hlopt{*} \hlnum{0.9}
\hldef{\}}

\hlkwd{cat}\hldef{(}\hlsng{"w:"}\hldef{,} \hlkwd{paste}\hldef{(}\hlkwd{round}\hldef{(w,} \hlnum{3}\hldef{),} \hlkwc{collapse} \hldef{=} \hlsng{", "}\hldef{),} \hlsng{" b:"}\hldef{,} \hlkwd{round}\hldef{(b,} \hlnum{3}\hldef{),} \hlsng{"\textbackslash{}n"}\hldef{)}
\end{alltt}
\begin{verbatim}
...
\begin{frame}{R (base): decision boundary and margins}

\end{frame}

\maxFrameImage{FIGS/L12-plot-svm-base-result-1.pdf}
...
\maxFrameImage{FIGS/L12-plot-svm-example-result-1.pdf}
...
\maxFrameImage{FIGS/L12-plot-svm-example-result-2-1.pdf}
...
\maxFrameImage{FIGS/L12-plot-svm-example-result-3-1.pdf}
...
\begin{frame}{R: plot decision boundaries and support vectors}

\end{frame}

\maxFrameImage{FIGS/L12-svm-soft-example-plots-1.pdf}
...
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{2025}\hldef{)}
\hldef{tune.out} \hlkwb{=} \hlkwd{tune}\hldef{(svm, y} \hlopt{~} \hldef{.,} \hlkwc{data} \hldef{= dat,} \hlkwc{kernel} \hldef{=} \hlsng{'linear'}\hldef{,} \hlkwc{ranges} \hldef{=} \hlkwd{list}\hldef{(}\hlkwc{cost} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0.01}\hldef{,} \hlnum{0.1}\hldef{,} \hlnum{1}\hldef{,} \hlnum{10}\hldef{,} \hlnum{100}\hldef{)))}
\hlkwd{print}\hldef{(tune.out)}
\hldef{best} \hlkwb{=} \hldef{tune.out}\hlopt{$}\hldef{best.model}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{frame}

\begin{frame}{R: final fit and visualization}

\end{frame}

\maxFrameImage{FIGS/L12-svm-soft-final-1.pdf}
