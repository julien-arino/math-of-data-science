...
\begin{frame} 
	\begin{definition}[Articulation set]
		For a connected graph, a set $X$ of vertices is called an \defword{articulation set} (or a \defword{cutset}) if the subgraph of $G$ generated by $A-X$ is not connected
	\end{definition}
	\vfill
...
\begin{frame}\frametitle{Adjacency matrix}
	...
	\begin{definition}[Adjacency matrix]
	The \defword{adjacency matrix} $M=M(D)=[m_{ij}]$ is a $p\times p$ matrix in which
	$$m_{ij}=\left \{ 
	\begin{array}{cl}
