...
\begin{itemize}
\item "Numerical dates" are in the form YYYY-MM-DD (e.g, these slides were compiled on 2026-09-10)
\vfill
...
\begin{itemize}
\item Because of the ongoing renovation of Machray Hall, I am sharing an office with 8 colleagues, connected to another office shared by 8 colleagues
\vfill
\item $\implies$ it is \textbf{impossible} to see you in my office
\vfill
\item I have booked 316 St Paul's from 1400 to 1800 on Tuesday for office hours
\vfill
\item Seeing you outside of office hours is impossible without ample warning, since I actually need to book a room
\end{itemize}
\end{frame}
...
\vfill
\item Three (3) Lab Tests (45 minutes) will be held during tutorials (dates later in the slides)
\end{itemize}
\end{frame}

\begin{frame}{Work in labs}
\begin{itemize}
\item The lecture notes contain problems that will be worked on in labs (when not in a Lab Test)
\vfill
\item During the tutorial, the TA will go over some of the problems
\vfill
\item A dedicated solutions manual will be posted on UMLearn 
\vfill
\item Solutions will only be posted after the corresponding tutorial period has ended
\end{itemize}
\end{frame}


\begin{frame}{R worksheets}
\begin{itemize}
\item Working on R coding is important in this course, although because of LLMs, it is not realistic anymore to use assignments to test this capacity
\vfill
\item Worksheets will be posted on UMLearn at the start of each week
\vfill
\item Solutions will be posted on UMLearn the next week
\vfill
\item They are meant as practice and will not be graded
\vfill
\item Lab Tests, the Midterm and Final Examinations will all contain \emph{one} question asking you to explain what a piece of R code does
\end{itemize}
\end{frame}
...
\vfill
\item 1 midterm on Thursday 5 November 2026 1800--2000 (\textbf{30\%})
\vfill
\item 1 final examination during the December 2026 Examination period (\textbf{40\%})
\end{itemize}
\vfill
\begin{itemize}
...
\vfill
\bbullet Tests your capacity to remember definitions or results, do simple proofs, carry out computations and explain R code
\end{frame}

\begin{frame}{Absences from scheduled evaluations}
\begin{itemize}
\item If you miss a lab test or the midterm examination, you can either use a self-declaration form or provide proper documentation (MD's note)
\vfill 
\item Self-declaration \textbf{must be made within 48 hours} of the event you are using it for
\vfill
\item They are intended for occasional and unforeseen circumstances, I \textbf{will not accept more than one during the term}
\vfill
\item You \textbf{must} use the \href{https://umanitoba.ca/student-supports/academic-supports/student-advocacy/self-declaration-policy-students}{form on this page} to self-declare your absence
\end{itemize}
\end{frame}

\begin{frame}{What if you missed an evaluation?}
If you have proper documentation (self-declaration or MD's note), then
\begin{itemize}
\item Lab Tests: the weight of the missed test is shifted to the final examination
\vfill
\item Midterm Examination: you will write a Deferred Midterm Examination (date set by Department of Mathematics)
\end{itemize}
\vfill
\alert{\textbf{Miss more than two (2)} evaluations (Lab Tests or Midterm) $\implies$ \textbf{F in the course}}
\end{frame}

\begin{frame}{Unhappy about a mark}
If you realise that some points were forgotten (this happens) or feel that you have been marked unfairly, feel free to reach \textbf{me} about it, preferably in person after class or during office hours
\vfill
\textbf{Do not bother the TAs} with this, they do not have the authority to change your marks
\vfill
\begin{itemize}
	\item I will only consider requests made within seven (7) calendar days from the date you receive your mark
	\vfill
	\item When processing requests, I \textbf{remark the entire examination}
	$\implies$ your overall mark could increase, stay the same \textbf{or even decrease}
\end{itemize}
\end{frame}

\begin{frame}{Colour coding in slides \& lecture notes}
...
\begin{frame}[plain]
  \begin{center}
    \includegraphics[width=\textwidth]{volunteer-notetaker-slide-2025.pdf}
  \end{center}
\end{frame}
...
All computer coding is in \texttt{R}
\vfill
...
\hlkwd{if} (condition_is_true) \{
  list of stuff to do
\} else \hlkwd{if} (another_condition) \{
  ...
\} else \{
  ...
\}
