...
\begin{tikzpicture}[
    ...
    \path (S1) edge [loop left] node[left] {$0.5$} (S1);
    \path (S1) edge [bend left=10] node[above,sloped] {$0.5$} (S3);
    \path (S2) edge [loop left] node[left] {$0.5$} (S2);
    \path (S2) edge [bend left=10] node[above] {$0.5$} (S3);
    \path (S3) edge [loop right] node[right] {$0.5$} (S3);
    \path (S3) edge [bend left=10] node[sloped,below] {$0.25$} (S1);
    \path (S3) edge [bend left=10] node[below] {$0.25$} (S2);
\end{tikzpicture}
\end{center}    
\end{frame}
    
\begin{frame}{Is this chain regular?}
    \begin{itemize}
