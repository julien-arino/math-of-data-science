...
\begin{itemize}
\item "Numerical dates" are in the form YYYY-MM-DD (e.g, these slides were compiled on 2025-08-31)
\vfill
...
\item If a self-declared absence overlaps with the due date and time of Friday at 1200
    \begin{itemize}
    \item submit the \href{https://forms.office.com/r/HSXE50PefN}{form} \textbf{prior} to the deadline
    \item This modifies \textit{your} deadline for \textit{that} assignment
    \end{itemize}
\item \textbf{I will not accept self-declarations after the deadline}: if at 1200 Friday, I have not received a self-declaration form XOR the assignment, you get a mark of zero on that assignment
\item Self-declarations are intended for very occasional and unforeseen circumstances $\implies$ I \textbf{will not accept more than two} during the term
\end{itemize}
\end{frame}
...
z = \hlkwd{c}(\hlsng{"red"}, \hlsng{"blue"})
> z
[1] \hlsng{"red"}  \hlsng{"blue"}
z = \hlkwd{c}(z, 1)
> z
[1] \hlsng{"red"}  \hlsng{"blue"} \hlsng{"1"}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
B <- \hlkwd{matrix}(\hlkwd{c}(1,2,3,4), nr = 2, nc = 2)
> B
     [,1] [,2]
[1,]    1    3
[2,]    2    4
C <- \hlkwd{matrix}(\hlkwd{c}(1,2,3,4), nr = 2, nc = 2, byrow = TRUE)
> C
     [,1] [,2]
[1,]    1    2
[2,]    3    4
...
\begin{itemize}
\item R does not have the keyword \texttt{end} to access the last entry in a matrix/vector/list..
\item Use \texttt{length} (lists or vectors), \texttt{nchar} (character chains), \texttt{dim} (matrices.. careful, of course returns 2 values)
\end{itemize}
\end{frame}
