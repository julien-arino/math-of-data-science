...
Let $\bx$ be an eigenvector of the largest eigenvalue $\lambda$ of the non-negative adjacency matrix $A$ of the undirected graph $G = (V, E)$.
\vfill
The \textbf{eigenvector centrality} (or \textbf{prestige score}) of vertex $i$ is equal to the leading eigenvector $x_i$ of the (column) stochastic matrix $N := AD^{-1}$:
\[
	N\bx = \bx
\]
\vfill
...
\item Let $D$ be the diagonal matrix of outdegrees where each element $D_{ii} = k_i$
\item Define a column stochastic matrix $N = AD^{-1}$
\item
