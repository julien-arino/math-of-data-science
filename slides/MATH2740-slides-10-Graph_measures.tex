...
\begin{frame}{Our example graph (rings a bell?)}
	\begin{center}
		\def\skip{2.75cm}
...
\[
	\frac {n(n-1)}{2}
\]
edges, while $G=(X,U)$ directed and simple of order $n$ has at most
\[
	n(n-1)
\]
arcs 
\vfill
Fraction of maximum number of edges or arcs present in the graph is the \textbf{density} of the graph, i.e., if the graph has $p$ edges or arcs, then its density is, respectively,
\[
	\frac {2p}{n(n-1)}
\]
or 
...
\begin{frame}{Example of density}
	\begin{center}
		...
		\end{center}
	\vfill
	Graph has order 6 and thus a max of 30 arcs. Here,8 arcs $\implies$ density 0.267
	\vfill 
